...
import { Users, CheckCircle2, Clock, Calendar, Timer, LogIn, LogOut } from "lucide-react";
...
function StatCard({
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-4 flex items-center gap-4">
...
function StaffDashboard({ user, onGoToAttendance }: { user: AuthUser; onGoToAttendance: () => void }) {
  ...
  const { record, checkIn, checkOut } = useCheckInOut();

  function loadWeek() {
    const weekStart = getWeekDates()[0];
    return api
      .get<{ records: AttendanceRecord[] }>("/attendance", { params: { from: weekStart, to: today } })
      .then((res) => setWeekRecords(res.data.records));
  }

  useEffect(() => {
    Promise.all([loadWeek(), api.get<{ employee: Employee }>("/employees/me")])
      .then(([, meRes]) => setJoinDate(toDateStr(new Date(meRes.data.employee.createdAt))))
  ...
  const statusBadge = checkOutTime
    ? { label: "Shift complete", classes: "bg-slate-100 text-slate-600" }
    : checkInTime
    ? { label: status, classes: "bg-green-50 text-green-700" }
    : { label: "Not checked in yet", classes: "bg-amber-50 text-amber-700" };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 max-w-lg">
        ...
        <h2 className="text-lg font-semibold text-slate-800 mb-3">{user.name}</h2>
        {initialLoading ? (
          <>
          ...
          <>
            <span
              className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${statusBadge.classes}`}
            >
              {statusBadge.label}
            </span>
            {timeAtWork && (
            ...
            <div className="flex flex-wrap items-center gap-2 mt-4">
              {!record ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg px-4 py-2"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium rounded-lg px-4 py-2"
                >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-medium rounded-lg px-4 py-2"
              >
                Go to My Attendance
              </button>
            </div>
